...
# Project-local agent rule files. During prepare Jikji inserts a small,
# clearly-delimited managed block that routes coding agents to `jikji find`
# before they fall back to native crawlers (grep/find/rg/search_files). The
# block is idempotent (updated in place) and removed again on `jikji clean`,
# preserving any surrounding user-authored content.
AGENT_RULES_FILES = ("AGENTS.md", "CLAUDE.md", ".cursorrules")
AGENT_RULES_BEGIN = "<!-- BEGIN JIKJI ROUTING (auto-generated by `jikji prepare`; do not edit inside this block) -->"
AGENT_RULES_END = "<!-- END JIKJI ROUTING -->"
_AGENT_RULES_BLOCK_RE = re.compile(
    re.escape(AGENT_RULES_BEGIN) + r".*?" + re.escape(AGENT_RULES_END) + r"\n?",
    re.DOTALL,
)
...
def _ignore_name(name: str, patterns: Iterable[str]) -> bool:
    ...
    if name == AGENT_DIR_NAME:
        return True
    if name in VISIBLE_MAP_NAMES or name in AGENT_RULES_FILES or name.startswith("Jikji_Report_"):
        return True
    ...
    for legacy_name in LEGACY_VISIBLE_MAP_NAMES:
        ...
        if legacy_map.exists():
            try:
                ...
                pass
    # Project-local agent rule files (AGENTS.md / CLAUDE.md / .cursorrules) so
    # coding agents route file discovery through `jikji find` instead of their
    # native crawlers. Opt out with config.write_agent_rules = False.
    if getattr(config, "write_agent_rules", True):
        try:
            write_agent_rules_files(root)
        except OSError:
            pass
    if progress:
        progress(
...
def _agent_rules_block_body() -> str:
    return (
        "## File & document search: use Jikji first\n\n"
        "This directory is indexed with **Jikji** "
        "(https://github.com/NomaDamas/jikji) for fast, low-token file discovery.\n\n"
        "- To locate files, folders, or document contents, run "
        "`jikji find . \"<natural language clue>\" --json` **first**.\n"
        "- Do NOT start with raw `find`, `grep`, `rg`, `ls`, `fd`, or a native "
        "`search_files` tool. Jikji has already indexed this directory and returns "
        "a small, ranked candidate slate in a single call, so a raw crawl just "
        "redoes work Jikji finished and bloats your context window.\n"
        "- Use the returned `answer_paths[]` / `candidates[].p` as the answer "
        "paths; open the original files only to verify the top results.\n"
        "- Follow `handoff_action`: `direct_use` (use the slate as-is), "
        "`jikji_retry` (exactly one sharper `jikji find` query), "
        "`raw_fallback_after_retry` (raw search only after that retry stayed empty "
        "or clearly wrong).\n"
        "- If this directory is not prepared yet, run `jikji prepare . --json`.\n"
    )


def _agent_rules_block() -> str:
    return f"{AGENT_RULES_BEGIN}\n{_agent_rules_block_body()}{AGENT_RULES_END}\n"


def write_agent_rules_files(root: Path) -> list[str]:
    """Insert/update the Jikji routing block in project-local agent rule files.

    Existing files keep their user-authored content; only the marker-delimited
    block is created or refreshed. Missing files are created with just the block.
    Returns the relative names that now carry the block.
    """
    block = _agent_rules_block()
    written: list[str] = []
    for name in AGENT_RULES_FILES:
        path = root / name
        if path.exists():
            try:
                ...
                continue
            if _AGENT_RULES_BLOCK_RE.search(current):
                updated = _AGENT_RULES_BLOCK_RE.sub(block, current, count=1)
            elif current.strip():
...
def strip_agent_rules_files(root: Path, *, dry_run: bool = False) -> dict[str, list[str]]:
    """Remove the Jikji routing block from project-local agent rule files.

    User-authored content is preserved. A file that contained only the Jikji
    block (i.e. Jikji created it) is removed entirely. Returns the affected
    paths split into ``edited`` (block stripped, file kept) and ``removed``.
    """
    edited: list[str] = []
    removed: list[str] = []
    for name in AGENT_RULES_FILES:
        ...
        try:
            ...
            continue
        if not _AGENT_RULES_BLOCK_RE.search(current):
            continue
        stripped = _AGENT_RULES_BLOCK_RE.sub("", current)
        if stripped.strip():
            edited.append(str(path))
            if not dry_run:
                _atomic_write_text(path, stripped.rstrip("\n") + "\n")
        else:
            ...
            if not dry_run:
                try:
                    ...
                    pass
    return {"edited": edited, "removed": removed}
